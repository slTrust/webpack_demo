...
module.exports = {
  plugins: [
    new HtmlWebpackPlugin({
      ...
      template: 'index.template.html' // 根据此模版生成 HTML 文件
    })
  ]
}
```
...
1. 运行 npm run build
2. 在dist目录自动生成了 index.html
3. 在浏览器中打开 index.html 文件，打开控制台也发现有输出，OK，自动生成 HTML 文件成功
4. 细心的朋友可能会发现一个问题，生成后的 HTML 文件引入的 JS 为绝对路径，但是真实的项目打完包之后都是部署在服务器上，用绝对路径肯定不行，因为你本地电脑的绝对路径放在服务器上肯定找不到
5. 我们要将引入的 js 文件从绝对路径改为相对路径
